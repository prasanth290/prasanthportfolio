...
async function main() {
  ...
  const p1 = await prisma.project.create({
    data: {
      ...
      shortDesc:
        "Real completed property management platform featuring automated tenant rent collection, online lease onboarding, maintenance ticket queues, and financial P&L reporting.",
...
You can test the actual deployed platform right now via the Live Demo button using pre-configured landlord credentials.`,
  ...
  const p2 = await prisma.project.create({
    data: {
      ...
      shortDesc:
        "High-throughput inventory control web app built for multi-location stock tracking, barcode receiving, dynamic low-stock reorder triggers, and supplier PO workflows.",
...
Test-drive the live warehouse manager portal using the credentials provided below.`,
  ...
  const p3 = await prisma.project.create({
    data: {
      ...
      shortDesc:
        "Full-featured appointment booking platform built with staff shift management, client CRM histories, SMS deposit reminders, and payment gateway integration.",
...
Test the live customer booking flow and manager dashboard via the demo link.`,
  ...
  const p4 = await prisma.project.create({
    data: {
      ...
      shortDesc:
        "High-performance business intelligence dashboard aggregating live API metrics, subscription cohort analytics, and automated reporting pipelines.",
...
PulseAnalytics is a custom internal tool engineered to aggregate live system telemetry, subscription revenue, and user cohort health into a unified control center.`,
  ...
  console.log("Seeded Proof-First Projects:", [p1.title, p2.title, p3.title, p4.title]);
